...
int main( int argc, char *argv[] )	  
{
  ...
  ackPacket.checksum = checksum( (char*) &(ackPacket.dPacket), sizeof( ackPacket.dPacket ) );
  while ( 1 ) {
    if ( ( numbytes = recvfrom( sockfd, packetBuf, sizeof( struct packet ), 0,
				&their_addr, &addr_len ) ) == -1 ) {
      perror( "recvfrom" );
    ...
    p = (struct packet*) packetBuf;
    checksumValue = checksum( (char*) &(p->dPacket), sizeof( p->dPacket) );
    if ( p->checksum == checksumValue && p->dPacket.seqNum == expectedSeqNum ) {
      printf( "Successfully Received Packet with Sequence Number %i\n", p->dPacket.seqNum );
    ...
    // if received corrupted or packet with wrong sequence number
      ...
      if ( p->checksum != checksumValue ) {
	printf( "Received Packet failed checksum. Checksum was %i but got %i\n",
